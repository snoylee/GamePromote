...
public class JsonUtil {
	...
	public Map<String, Object> fromJsonObject(JSONObject jsonObjectValue) {
		...
		try {
			...
			for (int i = 0; i < jsonobject.length(); i++) {
				...
				strvalue = jsonobject.get(name).toString();
				extractedValue = extractJsonRightValue(strvalue);
				if (extractedValue instanceof List||extractedValue instanceof Map) {
					structureMap.put(name, extractedValue);
				} 
					rawMap.put(name, strvalue);
	...
	Map<String, Object> structureMap = new TreeMap<String, Object>();
	...
	public Object findJsonNode(String exactname, JSONArray rootObject) {
		if (rootObject instanceof JSONArray) {
			JSONArray jsonobject = rootObject;
			for (int i = 0; i < jsonobject.length(); i++) {
				try {
	...
	private Object getNode(JSONObject rootObject, String data) throws JSONException {
		Object jsonobject;
		...
		if (ns.length > 1) {
			...
			if (((JSONArray) jsonobject).length() + 1 >= i) {
				jsonobject = ((JSONArray) jsonobject).get(Integer.parseInt(index));
			}
		}
		return jsonobject;
	}

	public Object from(String toString) {
		if (toString.trim().startsWith("[")) {
			return fromJsonArray(toString);
		} else if (toString.trim().startsWith("{")) {
			return fromJsonObject(toString);
		}
		return toString;
	}

	public Object toJson(String toString) {
		try {
			if (toString.trim().startsWith("[")) {
