...
public class MapAdapter extends BaseAdapter implements ExpandableListAdapter {
    ...
    public boolean isAdaptEmpty() {
        return this.fieldnames == null || this.fieldnames.size() == 0 || this.adaptInfo == null;
    }
    ...
    public void setAdaptInfo(AdaptInfo adaptInfo) {
        ...
        if (this.adaptInfo.pagefooterId != 0) {
            VisitUnit visitUnit;
            this.footerView = new ViewBinder(context, visitUnit = new VisitUnit()).inflate(this.adaptInfo.pagefooterId, null);
        }
    ...
    public void deployAdapter(int id) {
        ...
        if (adaptInfo != null) {
            ...
            ((AbsListView) ((Activity) context).findViewById(id)).setAdapter(this);
            ((AbsListView) ((Activity) context).findViewById(id)).setSelection(scrollpos);
            notifyDataSetChanged();
    ...
    public void deployListener(ActionListener listener) {
        ...
        if (!listenerMaps.containsKey(listener.getResrcId())) {
            listenerMaps.put(listener.getResrcId(), new ListenerBox(this, listener).setBasicAdapter(this));
        } else {
    ...
    private boolean isPaged() {
        return adaptInfo != null && this.adaptInfo.pagefooterId != 0 && this.adaptInfo.pageSize != -1;
    }
    ...
    int forceHeight = -1;
    private Map<View, VisitUnit> visits = new HashMap<View, VisitUnit>();

    public void putViewVisit(View view, VisitUnit visit) {
        visits.put(view, visit);

    }

    public Map<View, VisitUnit> getVisits() {
        return visits;
    }
    ...
    public View createItemView(int position) {
        ...
        if (getItem(position) instanceof Map) {
            ...
            visit.getDataUnit().getRepo().putAll(((Map) getItem(position)));
            itemView = new ViewBinder(context, visit).inflate(itemLayout, null);
            visit.setDisplay(itemView);
            putViewVisit(itemView, visit);
        } else {
            itemView = LayoutInflater.from(context).inflate(itemLayout, null);
        }
    ...
    protected void getViewInDetail(Object item, int position, View convertView) {
        ...
        if (item instanceof Cursor || item instanceof SQLiteCursor) {
            ...
            if (viewsid == null || viewsid.size() == 0) {
                if (visits.containsKey(convertView)) {
                    visits.get(convertView).getDataUnit().getRepo().putAll((Map<? extends String, ?>) getItem(position));
                    FillingController.parseParentFragmentFillingArea(visits.get(convertView), item);
                }
        ...
        // }
            ...
            treatString(item, convertView, position);
        } else {
    ...
    private void treatString(Object item, View convertView, int position) {
        setView(position, item, item, convertView, convertView.findViewById(viewsid.get(0)));
//        findAndBindView(convertView, position, item,"", item);
    }
    ...
    protected boolean setView(int pos, Object item, Object value, View convertView, View theView) {
        ...
        if (theView instanceof base.action.widget.ImageView&&value.toString().startsWith("http")) {
            if (value instanceof String) {
                ...
                // Logs.e(" --------------------------- imgurl " + value
                // + " item " + item);
                if (value != null && !value.equals("") && !value.equals("anyType{}")) {
                    try {
                        ...
                        loadImage(pos, item, value, convertView, theView, strurl);
            ...
            if (theView instanceof CheckBox) {
                ...
                ((TextView) theView).setText(
                        value instanceof SpannableStringBuilder ? (SpannableStringBuilder) value : value.toString());
                return true;
            }
    ...
    public void loadImage(int pos, Object item, Object value, View convertView, View theView, String strurl) {
        // TODO Auto-generated method stub
    ...
    public View getView(Object item, View convertView, int position, ViewGroup parent) {
                        ...
                        switch (actionType) {
                            ...
                            case ActionListener.OnCheckChanged:
                                if (view instanceof CheckBox) {
                                    ((CheckBox) view).setOnCheckedChangeListener(instance);
                                }
                                break;
                        }
                    }
...
//			getVisits().
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        getViewInDetail(item, position, convertView);
        int getcount = inpage ? this.getItemDataSrc().getCount() - 1 : this.getCount() - 1;
    ...
    private void addStyles(int position, View convertView) {
        for (Entry<Integer, StyleBox> en : styleMaps.entrySet()) {
            ...
            Map<Integer, Style> styles = (position == clickedItem ? stylebox.selectedStyle : stylebox.handlers);
            for (Style style : styles.values()) {
                switch (style.styleitem) {
                    case Style.BACKGROUND_COLOR:
                        if (style.value instanceof Integer) {
                            view.setBackgroundColor(context.getResources().getColor((Integer) style.value));

                        } else if (style.value instanceof String) {
                            view.setBackgroundColor(Color.parseColor((String) style.value));
                        }
                        break;
                    ...
                    case Style.TEXT_COLOR:
                        if (view instanceof TextView) {
                            ...
                            if (style.value instanceof Integer) {
                                ((TextView) view).setTextColor(context.getResources().getColor((Integer) style.value));
                            } else if (style.value instanceof String) {
                                ((TextView) view).setTextColor(Color.parseColor((String) style.value));
                            }
                        } else {
                            try {
                                throw new Exception("TEXT_COLOR's view is not isntanceof TextView but " + view.getClass());
                            } catch (Exception e) {
                                // TODO Auto-generated catch block
                                e.printStackTrace();
                            }
    ...
    public static abstract class ActionListener {
        ...
        public abstract void handle(MapAdapter mapAdapter, View view, int pos, ListenerBox listenerBox);
    ...
    public static class AdaptInfo {
        ...
        public static Integer[] transferStringIds(String[] sids) {
            Integer[] ids = new Integer[sids.length];
            for (int i = 0; i < sids.length; i++) {
                if (sids[i].equals("")) {
                    continue;
                }
                ids[i] = RRes.get("R.id." + sids[i]).getAndroidValue();
            }
            return ids;
        }
        ...
        public Set<ActionListener> actionListeners = new HashSet<ActionListener>(0);// varied
        // action
        // listeners
        ...
        public void appendResCntPair(int id, String item) {
            if (objectFieldList.size() == 0) {
                objectFieldList.addAll(Arrays.asList(objectFields));
            }
    ...
    public static abstract class ContinueRunner {
        ...
        public abstract void run(MapAdapter mapAdapter, Object param, Object lastItm);
