...
import com.xygame.sg.activity.notice.NoticePaymentZhuiJiaActivity;
 ...
 * @action [模特编辑头像任务]
 */
public class JieSuanZhuiJiaPayTask extends NetWorkUtil {

	private NoticePaymentZhuiJiaActivity activity;
	...
	public Object run(String methodname, List<String> params, Param aparam) {
		// TODO Auto-generated method stub
		activity = ((NoticePaymentZhuiJiaActivity) aparam.getActivity());
		String noticeId = activity.getnoticeId();
		String memId = activity.getmemId();
		String opsPassword=activity.getpayPassword();
		if(opsPassword!=null){
			params.add("opsPassword=" + opsPassword);
		}
		float amount=activity.getPayPrice();
		if(amount>0){
			params.add("amount="+String.valueOf(new Float(amount*100).intValue()));
		}
		...
		String usePerpay=String.valueOf(activity.isUsePerpay());
		params.add("noticeId=" + noticeId);
		params.add("memId=" + memId);
		params.add("useBanlance=" + useBanlance);
		params.add("usePerpay=" + usePerpay);
		ShowMsgDialog.show(activity, "正在请求中...", false);
