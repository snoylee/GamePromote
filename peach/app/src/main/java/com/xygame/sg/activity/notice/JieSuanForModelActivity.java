...
import com.xygame.sg.activity.notice.adapter.JieSuanForModelAdapter;
import com.xygame.sg.activity.notice.adapter.YiJieSuanForModelAdapter;
...
public class JieSuanForModelActivity extends SGBaseActivity implements OnClickListener{
	/**
	 * 公用变量部分
	 */
	...
	private JieSuanForModelAdapter daiAdapter;
	private YiJieSuanForModelAdapter yiAdapter;
	...
	private void initDatas() {
		...
		daiAdapter=new JieSuanForModelAdapter(this, null,noticeId);
		yiAdapter=new YiJieSuanForModelAdapter(this, null,noticeId);
	...
	private void loadDaiJieDatas() {
		...
		new Action("#.notice.LoadMemberPayForModelTask(${memberPay})", this, null, visit).run();
	}
	...
	public void loadPayedMemberDatas() {
		// TODO Auto-generated method stub
		VisitUnit visit = new VisitUnit();
		new Action("#.notice.LoadMemberPayForModelTask2(${memberPay})", this, null, visit).run();
	}

	public void finishLoadMemberPays(List<Map> map,String currTime) {
		// TODO Auto-generated method stub
		daiAdapter.clearDatas();
		List<NoticeStatusBeanForModel> datas=new ArrayList<NoticeStatusBeanForModel>();
		for(Map sMap:map){
			...
			it.setReward(String.valueOf(StringUtil.getPrice(Long.parseLong(sMap.get("reward").toString()))));
			...
			it.setApplyDesc(sMap.get("applyDesc").toString());
			it.setStatus(sMap.get("status").toString());
			it.setCount(sMap.get("count").toString());
			it.setGender(sMap.get("gender").toString());
			it.setFinalAmount(String.valueOf(StringUtil.getPrice(Long.parseLong(sMap.get("finalAmount").toString()))));
	...
	public void finishLoadPayedMember(List<Map> map) {
		...
		for(Map sMap:map){
			...
			it.setOrders(sMap.get("orders").toString());
			it.setRecruitId(sMap.get("recruitId").toString());
			it.setReward(String.valueOf(StringUtil.getPrice(Long.parseLong(sMap.get("finalAmount").toString()))));
			it.setUserIcon(sMap.get("userIcon").toString());
			it.setUserId(sMap.get("userId").toString());
			it.setUserNick(sMap.get("userNick").toString());
	...
	public void registerBoradcastReceiver() {
		...
		myIntentFilter.addAction("com.sg.jiesuan.pay.scuess.formodel.action");
	...
	private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			}else if("com.sg.jiesuan.pay.scuess.formodel.action".equals(intent.getAction())){
				String userId=intent.getStringExtra("userId");
				String liuYan=intent.getStringExtra("liuyan");
				daiAdapter.updateJiSuanItem(userId,liuYan);
			}
		}
	};
}
