...
public class EditorStyleActivity extends SGBaseActivity implements OnClickListener, OnItemClickListener {
	...
	private List<StyleBean> otherChannelList, userChannelList, keepDatas, selectDatas;
	...
	private void initDatas() {
		// TODO Auto-generated method stub
		titleName.setText(getResources().getString(R.string.sg_editor_style_title));
		...
		if (tsBean != null) {
			List<ModelStyleVo> tempDatas = tsBean.getStyleList();
			if (tempDatas != null) {
				initBinnar(tempDatas);
			}
		}
		loadModelStyle();
	}
	 ...
	 * @see [类、类#方法、类#成员]
	 */
	private void initBinnar(List<ModelStyleVo> tempDatas) {
		if (typeFlag != null) {
			if ("male".equals(typeFlag)) {
				for (int i = 0; i < tempDatas.size(); i++) {
					ModelStyleVo it = tempDatas.get(i);
					if ("1".equals(it.getExclusType()+"")) {
						...
						bean.setStyleId(it.getStyleId()+"");
						bean.setStyleName(it.getStyleName());
						bean.setSelected(1);
						bean.setOrderId(i + 1);
	...
	private void transferLocationService() {
		...
		new Action("#.personal.EditorUserStyle(${addModelStyle})", this, null, visit).run();
	}
	...
	private void loadModelStyle() {
		// TODO Auto-generated method stub
		VisitUnit visit = new VisitUnit();
		new Action("#.personal.LoadUserStyle(${queryModelStyleType})", this, null, visit).run();
	}
	...
	public void onItemClick(AdapterView<?> parent, final View view, final int position, long id) {
		...
		case R.id.otherGridView:
			if (userAdapter.getCount() == 3) {
				Toast.makeText(getApplicationContext(), "最多选三个标签", Toast.LENGTH_SHORT).show();
			} else {
	...
	public void onClick(View v) {
		...
		case R.id.rightButton:
			if (userAdapter.getCount() > 0) {
				...
				Toast.makeText(getApplicationContext(), "至少添加一个标签", Toast.LENGTH_SHORT).show();
			}
			break;
