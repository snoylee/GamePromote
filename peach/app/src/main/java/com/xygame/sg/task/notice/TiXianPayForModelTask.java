...
import com.xygame.sg.activity.personal.TiXianActivityForModel;
 ...
 * @action [模特编辑头像任务]
 */
public class TiXianPayForModelTask extends NetWorkUtil {
	
	private TiXianActivityForModel activity;
	/**
	 * 重载方法
	 * 
	 * @param result
	...
	public void callback(Param aparam, Object object) {
		...
		if (Constants.RESULT_CODE.equals(resultCode)) {
			String flag=aparam.getResultunit().getRawMap().get("record");
			if("1".equals(flag)){
				activity.finishTiXian();
			}else {
				Toast.makeText(aparam.getActivity(), flag, Toast.LENGTH_SHORT)
						.show();
			}
		}else {
	...
	public Object run(String methodname, List<String> params, Param aparam) {
		// TODO Auto-generated method stub
		activity = ((TiXianActivityForModel) aparam.getActivity());
		String userId=UserPreferencesUtil.getUserId(activity);
		String amount=activity.gettixianMoney();
