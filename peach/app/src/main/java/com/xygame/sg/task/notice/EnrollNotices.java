...
public class EnrollNotices extends NetWorkUtil {
	...
	public void callback(Param aparam, Object object) {
		...
		String codeError=aparam.getResultunit().getRawMap().get("code");
		if (Constants.RESULT_CODE.equals(resultCode)) {
			NoticeDetailActivity noticeDetailActivity = (NoticeDetailActivity) aparam.getActivity();
			noticeDetailActivity.responseEnroll();
		}else if("4011".equals(codeError)){
			NoticeDetailActivity noticeDetailActivity = (NoticeDetailActivity) aparam.getActivity();
			noticeDetailActivity.showDialogForRecord();
		}else{
