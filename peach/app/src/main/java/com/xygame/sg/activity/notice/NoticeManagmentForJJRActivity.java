 ...
 * @action [作品相册页面]
 */
public class NoticeManagmentForJJRActivity extends SGBaseActivity implements OnClickListener,PullToRefreshBase.OnRefreshListener2 {

	private TextView titleName, daiSView, zhaoMView, closeView;
	...
	private int pageIndexDai = 1,pageIndexZhao = 1,pageIndexClose = 1;//当前显示页数
	private boolean isLoadingDai=true,isLoadingZhao=true,isLoadingClose=true;
	private PullToRefreshListView2 daiSList, zhaoMList, closeList;
	private int loadFlag = 0;// 通告状态：0：待审核；1：招募中；2：拍摄中；3：已完成；4：已关闭
	private NoticeAdapter daiSAdapter, zhaoMAdapter, closeAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(new ViewBinder(this, visitUnit).inflate(R.layout.notice_jjr_managment_layout, null));
		InitImageView();
		initViews();
		initListeners();
		initDatas();
	...
	private void initDatas() {
		...
		zhaoMAdapter = new NoticeAdapter(this,null);
	...
	private void loadDatas(int pageindex){
		...
		item.setServiceURL(ConstTaskTag.QUEST_JJR_NOTICE_MANAGE);
		ThreadPool.getInstance().excuseAction(this, item, ConstTaskTag.RESPOSE_JJR_NOTICE_MANAGE);
	}
	...
	protected void getResponseBean(ResponseBean data) {
		...
		switch (data.getPosionSign()){
			case ConstTaskTag.RESPOSE_JJR_NOTICE_MANAGE:
				if ("0000".equals(data.getCode())){
					parseRequestData(data);
				}else{
					Toast.makeText(NoticeManagmentForJJRActivity.this,data.getMsg(), Toast.LENGTH_SHORT).show();
				}
				break;
		}
	}
...
//			startActivityForResult(intent,1);
	...
	public void toNoticeStatus(JJRNoticeBean item) {
		...
		Intent intent = new Intent(NoticeManagmentForJJRActivity.this, NoticeStatusActivity.class);
		intent.putExtra("noticeId", item.getNoticeId());
		startActivity(intent);
				...
				case 0:
					...
					if (!TextUtils.isEmpty(notices)&&!"[]".equals(notices)){
						...
						for (int i=0;i<array.length();i++){
							...
							JJRPublisher bean=new JJRPublisher();
							bean.setUserIcon(UserPreferencesUtil.getHeadPic(this));
							bean.setUserId(UserPreferencesUtil.getUserId(this));
							bean.setUsernick(UserPreferencesUtil.getUserNickName(this));
							bean.setUserType(Constants.CARRE_MERCHANT);
							item.setPublisher(bean);
							datas.add(item);
						}
					...
					break;
				case 1:
					List<JJRNoticeBean> datas1=new ArrayList<>();
					String notices1=data.getRecord();
					if (!TextUtils.isEmpty(notices1)&&!"[]".equals(notices1)){
						JSONArray array=new JSONArray(notices1);
						for (int i=0;i<array.length();i++){
							JSONObject subObj=array.getJSONObject(i);
							JJRNoticeBean item=new JJRNoticeBean();
							item.setNoticeContent(StringUtils.getJsonValue(subObj, "noticeContent"));
						...
						if (datas1.size()<pageSize){
							isLoadingZhao=false;
						}
						zhaoMAdapter.addDatas(datas1, pageIndexZhao);
					}else{
						isLoadingZhao=false;
					}
					break;
