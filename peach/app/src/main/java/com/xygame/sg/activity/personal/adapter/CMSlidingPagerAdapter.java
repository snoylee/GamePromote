...
public class CMSlidingPagerAdapter extends FragmentPagerAdapter {
	...
	public int getCacheCount() {
		return CMPageAdapterTab.values().length;
	}

	public CMSlidingPagerAdapter(FragmentManager fm, Context context, ViewPager pager,boolean isQuery) {
		super(fm);
		fragments = new ScrollTabHolderFragment[CMPageAdapterTab.values().length];
	...
	private void init(FragmentManager fm) {
		for (CMPageAdapterTab tab : CMPageAdapterTab.values()) {
			try {
	...
	public CharSequence getPageTitle(int position) {
		CMPageAdapterTab tab = CMPageAdapterTab.fromTabIndex(position);
