 ...
 */
public class SGBaseActivity extends FragmentActivity implements Handler.Callback {
...
//		queryCustomerServicePhone();
	...
	public void requestActType() {
		...
		try {
			...
			ThreadPool.getInstance().excuseAction(this, item, ConstTaskTag.QUERY_BAIDU_LOCATION);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
	}
...
//				Toast.makeText(this,"设备号上传成功",Toast.LENGTH_SHORT).show();
			...
			case ConstTaskTag.QUERY_ACT_MIN_PRICE:
				...
				break;
			case ConstTaskTag.QUERY_BAIDU_LOCATION:{
