 ...
 * @action [作品相册页面]
 */
public class PlushJJRNoticeActivity extends SGBaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(new ViewBinder(this, visitUnit).inflate(R.layout.sg_plush_jjr_notice_layout, null));
...
//			}
	...
	private void jjrPulsh() {
		ShowMsgDialog.showNoMsg(this,false);
		RequestBean item = new RequestBean();
		try {
			item.setData(new JSONObject().put("publishUserId",UserPreferencesUtil.getUserId(this)).put("subject",pnBean.getCameraTheme()).put("noticeContent", pnBean.getNoticeTip()));
		} catch (Exception e1) {
			e1.printStackTrace();
		}
		item.setServiceURL(ConstTaskTag.QUEST_JJR_PLUSH_NOTICE);
		ThreadPool.getInstance().excuseAction(this,item,ConstTaskTag.RESPOSE_JJR_PLUSH_NOTICE);
	}
	...
	protected void getResponseBean(ResponseBean data) {
		...
		switch (data.getPosionSign()){
			case ConstTaskTag.RESPOSE_JJR_PLUSH_NOTICE:
				if ("0000".equals(data.getCode())){
					Intent intent = new Intent("com.xygame.jjren.plush.notice.action");
	...
	protected void responseFaith(ResponseBean data) {
		...
		Toast.makeText(this,"发布通告失败，请重试",Toast.LENGTH_SHORT).show();
	}
...
//				}
	...
	private boolean isTrue() {
		...
		if ("".equals(themeText.getText().toString())) {
			...
			if ("".equals(introduceText.getText().toString().trim())){
				Toast.makeText(getApplicationContext(), "通告备注不能为空", Toast.LENGTH_SHORT).show();
			}else {
				flag = true;
			}
		}
