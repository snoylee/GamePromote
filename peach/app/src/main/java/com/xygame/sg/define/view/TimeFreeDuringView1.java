 ...
 * @action [选择拍照或相册的界面]
 */
public class TimeFreeDuringView1 extends SGBaseActivity {
			...
			public void onClick(View arg0) {
				...
				if (isGo()){
					for (FreeTimeBean it:timers){
						if (it.isSelect()){
							timerDuringBean.setTimersOfDate(it);
							break;
						}
					}
					...
					Toast.makeText(TimeFreeDuringView1.this,"至少选择一项",Toast.LENGTH_SHORT).show();
				}
			}
		});
	}
	...
	private void changeStus(int index) {
		for (int i=0;i<timers.size();i++){
			if (index==i){
				timers.get(i).setIsSelect(true);
			}else{
				timers.get(i).setIsSelect(false);
			}
		}
		refreshViews();
	}
