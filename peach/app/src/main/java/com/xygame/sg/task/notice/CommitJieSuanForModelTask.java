 ...
 * @action [模特编辑头像任务]
 */
public class CommitJieSuanForModelTask extends NetWorkUtil {
	
	private ModelRequestJieSuanActivity activity;
	/**
	 * 重载方法
	 * 
	 * @param result
	...
	public Object run(String methodname, List<String> params, Param aparam) {
		// TODO Auto-generated method stub
		activity = ((ModelRequestJieSuanActivity) aparam.getActivity());
		String curUserId=UserPreferencesUtil.getUserId(activity);
		String opsPwd=activity.getPayPassword();
		String applyDesc=activity.getLiuYan();
		String id=activity.getMenId();
		String noticeId=activity.getNoticeId();
		params.add("curUserId="+curUserId);
		params.add("opsPwd="+opsPwd);
		if(!"".equals(applyDesc)){
			params.add("applyDesc="+applyDesc);
		}
		params.add("id="+id);
