 ...
 * @action [模特编辑头像任务]
 */
public class RegisterComposeTask extends NetWorkUtil {
	
	private MyComposeActivity activity;
	/**
	 * 重载方法
	 * 
	 * @param result
	...
	public Object run(String methodname, List<String> params, Param aparam) {
		// TODO Auto-generated method stub
		activity = ((MyComposeActivity) aparam.getActivity());
		String userId=UserPreferencesUtil.getUserId(activity);
		String couponCode=activity.getduiHuanMa();
		params.add("userId="+userId);
		params.add("couponCode="+couponCode);
		ShowMsgDialog.show(activity, "兑换中...", true);
