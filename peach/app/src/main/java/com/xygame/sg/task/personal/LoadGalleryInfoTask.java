 ...
 * @action [请添加内容描述]
 */
public class LoadGalleryInfoTask extends NetWorkUtil {

	private PersonalPhotoesBrowsersActivity activity;
...
//			activity.parseDatasRefresh(map);
		} else {
	...
	public Object run(String methodname, List<String> params, Param aparam) {
		// TODO Auto-generated method stub
		activity = ((PersonalPhotoesBrowsersActivity) aparam.getActivity());
		String galId=activity.getGlaryId();
		String userId = "";
		if (UserPreferencesUtil.isOnline(activity)){
			userId = UserPreferencesUtil.getUserId(activity);
		}
		String seeUserId=activity.getSeeUserId();
		params.add("userId="+userId);
		params.add("galId="+galId);
		params.add("seeUserId="+seeUserId);
//		ShowMsgDialog.show(activity, "相册请求中...", false);
