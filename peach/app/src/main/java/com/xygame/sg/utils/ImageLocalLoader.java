...
public class ImageLocalLoader
{
	...
	public static ImageLocalLoader getInstance()
	{
		...
		if (mInstance == null)
		{
			synchronized (ImageLocalLoader.class)
			{
				if (mInstance == null)
				{
					mInstance = new ImageLocalLoader(1, Type.LIFO);
				}
			}
		}
		return mInstance;
	...
	public Bitmap loadSingleImage(final String path, ImageView imageView)
	{
		ImageSize imageSize = getImageViewWidth(imageView);
	...
	public Bitmap loadImage(final String path, int reqWidth,int reqHeight)
	{
		Bitmap bm = decodeSampledBitmapFromResource(path, reqWidth,
				reqHeight);
		return bm;
