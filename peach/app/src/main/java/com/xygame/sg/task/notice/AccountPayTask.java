 ...
 * @action [模特编辑头像任务]
 */
public class AccountPayTask extends NetWorkUtil {
	
	private ZhuiJiaDialogActivity activity;
	/**
	 * 重载方法
	 * 
	 * @param result
	...
	public Object run(String methodname, List<String> params, Param aparam) {
		// TODO Auto-generated method stub
		activity = ((ZhuiJiaDialogActivity) aparam.getActivity());
		ZhuiJiaBean bean=activity.getZjBean();
		String[] memIds=bean.getMemIds().split(",");
		String status=bean.getStatus();
		String useBanlance=String.valueOf(true);
		params.add("status="+status);
		params.add("useBanlance="+useBanlance);
		params.add("memIds="+memIds);
