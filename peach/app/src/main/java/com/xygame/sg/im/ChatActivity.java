 ...
 * 聊天对话.
 */
public class ChatActivity extends SGBaseActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.le_chat_single);
        initViews();
        initListeners();
        initDatas();
    }
    ...
    private void initDatas() {
        ...
        try {
            if (SGApplication.getInstance().getConnection() != null) {
                chat = SGApplication.getInstance().getConnection().getChatManager()
                        .createChat(XMPPUtils.getUserJid(this, msgBean.getFriendUserId()), null);
            }
        } catch (Exception e) {
        ...
        transaction.commit();
        adapter = new MessageListAdapter(ChatActivity.this, null);
...
//		}
    ...
    private void uploadLocalStatus(SGNewsBean msgBean) {
        NewsEngine.updateChatNewsStatus(this, msgBean, UserPreferencesUtil.getUserId(this));
    }

    private void loadHistoryDatas() {
        MessageDatas = NewsEngine.quaryChatHistory(this, msgBean, UserPreferencesUtil.getUserId(this));
    }
...
//		startActivityForResult(openAlbumIntent, CHOOSE_PICTURE);
    ...
    private class MessageListAdapter extends BaseAdapter {
        ...
        class HoldView {
            ...
            TextView useridView, dateView, msgView, mapAdress, voiceTime, messageTimer;
            View soundView, mapImageLayout, photoView, otherView, sendFaith,playIcon;
            SeekBar skbProgress;
            ProgressBar sending;
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (Constants.NEWS_RECIEVE.equals(message.getInout())) {
                convertView = LayoutInflater.from(context).inflate(R.layout.le_formclient_chat_in, null);
            } else {
                convertView = LayoutInflater.from(context).inflate(R.layout.le_formclient_chat_out, null);
            ...
            if (Constants.SEND_TEXT.equals(message.getType())) {
                ...
                final String voidUrl=message.getMsgContent();
                if (voidUrl.contains("http://")) {
                    holdView.imageView.setImageResource(R.drawable.default_avatar);
                    videoImageLoader.DisplayImage(voidUrl, holdView.imageView);
                    holdView.imageView.setOnClickListener(new View.OnClickListener() {

                        @Override
                        public void onClick(View arg0) {
                            Intent intent = new Intent(context, VideoPlayerActivity.class);
                            intent.putExtra("vidoUrl", voidUrl);
                            context.startActivity(intent);
                        }
                    });
    ...
    private void alertDialog(final String content) {
        ...
        copyView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                StringUtil.copy(content, ChatActivity.this);
                dlg.dismiss();
            }
        });
...
//						initImageMessage(picPath);
    ...
    private class MovingMp4 implements Runnable {
        ...
        @Override
        public void run() {
            String mFileName = FileUtil.RECORD_ROOT_PATH.concat(Constants.getMP4Name(ChatActivity.this));
                ...
                switch (msg.what) {
                    ...
                    case 7:
                        if (Constants.SEND_IMAGE.equals(sendMessageType)) {
                            ...
                            NewsEngine.updateChatSendStatusForImages(ChatActivity.this, sendBean,
                                    UserPreferencesUtil.getUserId(ChatActivity.this));
                            adapter.updateItemSendStatusForImages(sendBean);
                            excuseSendPicAction();
                        } else {
                            sendBean.setMsgStatus(Constants.NEWS_SEND_FAITH);
                            NewsEngine.updateChatSendStatus(ChatActivity.this, sendBean,
                                    UserPreferencesUtil.getUserId(ChatActivity.this));
...
//					dealWithResult(resInt);
    ...
    private void initVidelMessage(String videoFile, String videoImage){
        ...
        try {
            ...
            // 新增本地数据
            NewsEngine.inserChatNew(this, sendBean);
...
//			message.setProperty("immessage.time", time);
            ...
            NewsEngine.updateChatSendContent(this, sendBean, UserPreferencesUtil.getUserId(this));
