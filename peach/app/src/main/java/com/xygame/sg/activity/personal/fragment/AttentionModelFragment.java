...
public class AttentionModelFragment extends AttentionBaseFragment {
    private PullToRefreshListView listView;
    private AttentionModelAdapter adapter;
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        myIntentFilter.addAction(Constants.MODEL_ATTENTION_STATUS_CHANGE);
        getActivity().registerReceiver(mBroadcastReceiver, myIntentFilter);
    }
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        View mView = new ViewBinder(this.getActivity(), unit.setDisplay(this)).inflate(R.layout.fragment_attention_model, null);
        initViews(mView);
        initDatas();
        initListeners();
        requestData();
    ...
    public void responseAttentionList(List<AttentUserView> resultAttentionList) {
        if (resultAttentionList != null) {
            if (mCurrentPage > 1) {
                ...
                adapter = new AttentionModelAdapter(getActivity(), attentionList);
    ...
    private BroadcastReceiver mBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (Constants.MODEL_ATTENTION_STATUS_CHANGE.equals(intent.getAction())) {
                mCurrentPage=1;
                queryBean.getPage().setPageIndex(mCurrentPage);
                requestData();
            }
