 ...
 * @action [请求阿里参数]
 */
public class ResponseServiceNumber extends NetWorkUtil {
	...
	public void callback(Param aparam, Object object) {
		...
		if (Constants.RESULT_CODE.equals(resultCode)) {
			...
			for(Map it:map){
				if("app_service_phone".equals(it.get("propName").toString())){
					String accessKey=it.get("propValue").toString();
//					UserPreferencesUtil.setServicePhone(activity,accessKey);
				}
			}
		}
	...
	public Object run(String methodname, List<String> params, Param aparam) {
		// TODO Auto-generated method stub
		activity = ((MainFrameActivity) aparam.getActivity());
