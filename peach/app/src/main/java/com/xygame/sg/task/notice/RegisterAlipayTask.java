 ...
 * @action [模特编辑头像任务]
 */
public class RegisterAlipayTask extends NetWorkUtil {
	
	private BangAlipayActivity activity;
	/**
	 * 重载方法
	 * 
	 * @param result
	...
	public void callback(Param aparam, Object object) {
		...
		if (Constants.RESULT_CODE.equals(resultCode)) {
			String record = aparam.getResultunit().getRawMap().get("record");
			if("1".equals(record)){
//				activity.bangInfo();
			}else{
				Toast.makeText(aparam.getActivity(), record, Toast.LENGTH_SHORT)
				.show();
			}
		}else {
	...
	public Object run(String methodname, List<String> params, Param aparam) {
		// TODO Auto-generated method stub
		activity = ((BangAlipayActivity) aparam.getActivity());
		String userId=UserPreferencesUtil.getUserId(activity);
		String accountName=activity.getName();
		String accountNo=activity.getalipayCount();
