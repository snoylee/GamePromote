 ...
 */
public class TempGroupMemberDeleAdapter extends BaseAdapter implements SectionIndexer{
    private Context context;
    private List<BlackMemberBean> vector;
    private ImageLoader mImageLoader;
    public TempGroupMemberDeleAdapter(Context context, List<BlackMemberBean> vector) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (null == convertView) {
            ...
            convertView = LayoutInflater.from(context).inflate(
                    R.layout.temp_group_dele_item, parent, false);
...
//        }
    ...
    public void updateItemStatus(BlackMemberBean item) {
    ...
    public List<BlackMemberBean> getSelectDatas() {
        List<BlackMemberBean> tempDatas=new ArrayList<>();
        for (BlackMemberBean it:vector){
            if (it.isSelect()){
                tempDatas.add(it);
            }
        }
    ...
    public void removing( List<UserBeanInfo> userBeanInfos) {
        for (UserBeanInfo it:userBeanInfos){
            for (int i=0;i<vector.size();i++){
                if (it.getUserId().equals(vector.get(i).getUserId())){
                    vector.remove(i);
                    break;
                }
            }
    ...
    private static class ViewHolder {
        ...
        public ImageView sexIcon,selectIcon;
