 ...
 */
public class SlideForGroupChatAdapter extends BaseAdapter implements SectionIndexer {
    ...
    public SlideForGroupChatAdapter(Context context, List<GZ_GroupBean> vector,SGNewsBean sendBean,TransferBean transferBean,GroupMemberBean currSelectedUnavilbeBean,List<GroupMemberBean> selectMembers0) {
        this.context = context;
        this.sendBean=sendBean;
        this.transferBean=transferBean;
        this.currSelectedUnavilbeBean=currSelectedUnavilbeBean;
    ...
    public void updateSelectMember(String grouId,List<GroupMemberBean> selectdMembers){
        for (int i=0;i<vector.size();i++){
            if (grouId.equals(vector.get(i).getId())){
                vector.get(i).setTempDatas(selectdMembers);
                break;
            }
        }
...
//        }
    ...
    public void setUnDividerGroup(List<GroupMemberBean> selectMembers0) {
        this.selectMembers0=selectMembers0;
    }
    ...
    private class AddClickListener implements JustClickListener{
        ...
        public void justClickAction() {
            ...
            itC.setId("0");
            itC.setName("未分组");
            itC.setTempDatas(selectMembers0);
            ...
            for (GZ_GroupBean its : getDatas()) {
                if (!item.getId().equals(its.getId())){
            ...
            transfer.setSelectMembers0(item.getTempDatas());
            Intent intent = new Intent(context, GZ_GroupListForGCActivity.class);
