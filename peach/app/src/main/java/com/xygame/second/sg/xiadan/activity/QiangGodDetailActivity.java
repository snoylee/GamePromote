...
public class QiangGodDetailActivity extends SGBaseActivity implements OnClickListener, ViewPager.OnPageChangeListener {
    /**
     * 公用变量部分
     */
    private TextView titleName, userName, sexAge, loactionTex, godAppName, userLable, priceValue, acceptTime, oralText;
    private View backButton, intoPersonal, sex_age_bg, bottomView, playButton, locationView,isShowVoiceView,playVoiceButton;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.qiang_god_detail_layout);
        initViews();
        initListensers();
        initDatas();
    }
...
//        intoPersonal.setOnClickListener(this);
    ...
    public void onClick(View v) {
        ...
        if (v.getId() == R.id.backButton) {
            ...
            if (!TextUtils.isEmpty(mp4Url)) {
                ...
                Constants.imageBrower(QiangGodDetailActivity.this,0, tempImages, false);
            }
            ...
            if (!CalendarUtils.isPassed10Min(UserPreferencesUtil.getorderExpireTime(this),timerCountBean.getStartTime())){
                commitOrderAction();
            }else{
                showDilog("抱歉！该订单已过期");
            }
    ...
    protected void getResponseBean(ResponseBean data) {
        ...
        switch (data.getPosionSign()) {
            ...
            case ConstTaskTag.QUERY_COMMIT_ORDER_QIANG:
                if ("0000".equals(data.getCode())) {
                    showComfirmDialog("订单已提交成功，请在"+UserPreferencesUtil.getpayExpireTime(this)+"分钟内完成支付操作");
                } else {
                    Toast.makeText(this, data.getMsg(), Toast.LENGTH_SHORT).show();
                }
                break;
    ...
    private void doAction(){
        ...
        String timerText=CalendarUtils.getYMDHMStr(Long.parseLong(blackMemberBean.getStartTime())).concat("  ").concat(blackMemberBean.getHoldTime()).concat("小时");
...
//            isCycle();
            if (noMp4){
                ...
                Constants.imageBrower(QiangGodDetailActivity.this, position - 1, tempImages, false);
            }else {
                if (position == 1) {
                    Intent intent = new Intent(QiangGodDetailActivity.this, VideoPlayerActivity.class);
                    intent.putExtra("vidoUrl", mp4Url);
                    startActivity(intent);
                } else {
                    ...
                    Constants.imageBrower(QiangGodDetailActivity.this, position - 2, tempImages, false);
                }
            }
        }
    };
